...
| `fileset-ok` | `download:request-start` → … → `download:complete` → `fetch-ok` | model `.task` 파일 fetch 실패 — HuggingFace 401/403, CDN 차단, redirect mismatch, 또는 first-chunk 후 30 초 stall (`download:idle-timeout`). Sprint 12 task-2 가 추가한 `download:request-start` / `download:response` / `download:first-chunk` / `download:milestone` / `download:complete` / `download:idle-timeout` 6 stage 가 어디서 멈췄는지로 정밀 압축 가능 — 상세 표는 [`docs/uat-troubleshooting.md` 6-B](./uat-troubleshooting.md#6-b-stage-별-root-cause-매핑-표) | model URL 직접 curl + `Content-Length` 확인. simctl log 의 마지막 `download:*` stage 확인 |
